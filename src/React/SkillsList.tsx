...
const SkillsList = () => {
  ...
  const skills = {
    "Développement Web": [
      "Applications Web Modernes (SPA, PWA)",
      "Interfaces Dynamiques & Réactives",
      "Sites Vitrines & Landing Pages Haute Performance",
    ],
    "Développement Logiciel": [
      "Apps Cross-Platform (Electron, Flutter, .NET MAUI)",
      "Microservices Architectures (Node.js, Go, Python)",
      "Systèmes Embarqués & IoT (Raspberry Pi, Arduino)",

    ],
    "Développement Mobile": [
      "Apps iOS & Android (Swift, Kotlin, Flutter, React Native)",
      "Jeux Mobiles Légers (Unity, Godot)",
      "Applications Métiers & SaaS Mobile",
    ],
  };
  ...
  return (
    <div className="text-left pt-3 md:pt-9 gap-6">
      <h3 className="text-[var(--white)] text-3xl md:text-4xl font-semibold md:mb-6">
      Mes compétences
      </h3>
      <ul className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4 text-lg">
      {Object.entries(skills).map(([category, items]) => (
        <li key={category} className="w-full">
        <div
          onClick={() => toggleItem(category)}
          className="bg-[#1414149c] rounded-2xl text-left hover:bg-opacity-80 transition-all border border-[var(--white-icon-tr)] cursor-pointer overflow-hidden"
        >
